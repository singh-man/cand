...
import com.cartrawler.assessment.service.impl.CarProcessingService;
...
public class AssessmentRunner {
    ...
    public static void main(String[] args) {
        ICarProcessing carProcessingService = new CarProcessingService(new CarResultCustomComparator());
        List<CarResult> processedCars = carProcessingService.processCars(CARS,
                new RemoveCarsByFuelPolicyAndAbovePrice(
                        CarResult.FuelPolicy.FULLFULL,
                        new IsBelowMedianPrice()));
        Display display = new Display();
        display.render(new LinkedHashSet<>(processedCars));

        //------------- Below is the customized order -------------
        // Strategy: ICarsFilter implementations
        // Decorator: Demo; of different ordering
        // Template: An order of Algo steps
        Function<Collection<CarResult>, Collection<CarResult>> filterCarsSteps =
        ...
        Collection<CarResult> apply = carProcessingService.processCarsDecorated(
                CARS,
                new RemoveCarsByFuelPolicyAndAbovePrice(
                        CarResult.FuelPolicy.FULLFULL,
                        new IsBelowMedianPrice()));
