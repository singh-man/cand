...
class ATMServiceTest {
    ...
    public void testWithdrawAmount() {
        Optional<UserAccount> accountOptional = userAccountRepository.findById(987654322L);
        if (accountOptional.isPresent()) {
            final WithdrawalRequest withdraw = new WithdrawalRequest(987654322, 1212, 120);
            TransactionDTO transactionDTO = atmService.withdrawAmount(withdraw);
            assertEquals(2, transactionDTO.getDispensedCashDto().getNote50());
            assertEquals(1, transactionDTO.getDispensedCashDto().getNote20());
            assertEquals(120.0, transactionDTO.getDispensedCashDto().getMoneyCount());
        }
    }
}